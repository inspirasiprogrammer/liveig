  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      if (status == "interrupted") {
        div_asistindo.innerHTML = "SLOW<span>.</span>";
      } else if (status == "stopped") {
        location.href = "stream/encerrar";
        return;
      } else {
        div_asistindo.innerHTML = "Live Count<span>" + viewCount + "</span>";
      }
...
function atualizarTRestante() {
  setInterval(function () {
    ...
    document.getElementById("tRestante").innerText = dif.toLocaleString(
      "pt-BR",
      options
    );
  }, 1000);
